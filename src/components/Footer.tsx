import { useTranslations } from "next-intl";
import { Link } from "@/i18n/navigation";

export default function Footer() {
  const t = useTranslations("footer");
  const nav = useTranslations("nav");

  return (
    <footer className="bg-brand-darker border-t border-brand-border/50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {/* Brand */}
          <div className="space-y-4">
            <img
              src="/logos/logo-full.png"
              alt="IntermosCraft"
              className="h-9 w-auto"
            />
            <p className="text-brand-muted text-sm leading-relaxed max-w-xs">
              {t("tagline")}
            </p>
          </div>

          {/* Navigation */}
          <div>
            <h3 className="text-brand-copper uppercase text-xs tracking-widest font-semibold mb-4">
              {t("navigation")}
            </h3>
            <ul className="space-y-2">
              {(["home", "about", "products", "custom", "contact"] as const).map(
                (key) => {
                  const href =
                    key === "home"
                      ? "/"
                      : (`/${key}` as
                          | "/about"
                          | "/products"
                          | "/custom"
                          | "/contact");
                  return (
                    <li key={key}>
                      <Link
                        href={href}
                        className="text-brand-muted hover:text-brand-text text-sm transition-colors"
                      >
                        {nav(key)}
                      </Link>
                    </li>
                  );
                }
              )}
            </ul>
          </div>

          {/* Social */}
          <div>
            <h3 className="text-brand-copper uppercase text-xs tracking-widest font-semibold mb-4">
              {t("followUs")}
            </h3>
            <div className="flex gap-4">
              <a
                href="https://instagram.com/intermoscraft"
                target="_blank"
                rel="noopener noreferrer"
                className="text-brand-muted hover:text-brand-copper transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://facebook.com/intermoscraft"
                target="_blank"
                rel="noopener noreferrer"
                className="text-brand-muted hover:text-brand-copper transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385h-3.047v-3.47h3.047v-2.642c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953h-1.514c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385c5.738-.9 10.126-5.864 10.126-11.854z" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="mt-12 pt-8 border-t border-brand-border/30 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-brand-subtle text-xs">
            &copy; {new Date().getFullYear()} IntermosCraft. {t("rights")}
          </p>
          <div className="flex gap-6 text-xs text-brand-subtle">
            <span className="hover:text-brand-muted cursor-pointer transition-colors">
              {t("privacy")}
            </span>
            <span className="hover:text-brand-muted cursor-pointer transition-colors">
              {t("terms")}
            </span>
          </div>
        </div>
      </div>
    </footer>
  );
}
